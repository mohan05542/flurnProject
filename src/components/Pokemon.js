...
const pokemon = ({ pokemon }) => {
    ...
    return (
        <>
            <Card className={`${pokemon.types[0].type.name} rounded my-3 p-3 rounded text-center shadow p-3 mb-5  `} style={{ border: 'none' }}>
                <Link to={`/pokemon/${pokemon.id}`}>
                    <Card.Img style={{ width: '8rem' }} src={pokemon.sprites.front_default} variant='top'/>
                </Link>
                <Card.Body className={`${pokemon.types[0].type.name} text-white`}>
